...
def filter_files_by_pp_rank(
    hf_weights_files: list[str], hf_folder: str, index_file: str
) -> list[str]:
    """Filter safetensors files to only include those needed by this PP rank.
    This reduces page cache from full model size to per-PP-rank size."""
    index_file_name = os.path.join(hf_folder, index_file)
    if not os.path.isfile(index_file_name):
        return hf_weights_files
    ...
    for name, file_name in weight_map.items():
        ...
        if is_needed:
            needed_files.add(os.path.join(hf_folder, file_name))

    filtered = [f for f in hf_weights_files if f in needed_files]
    if len(filtered) == 0:
        return hf_weights_files
    return filtered
        ...
        if safetensors_load_strategy == "eager":
            ...
            with safe_open(st_file, framework="pt") as f:
                for name in f.keys():  # noqa: SIM118
                    ...
                    yield name, param
            # Evict page cache for this file (Grace Blackwell unified memory)
            try:
                _fd = os.open(st_file, os.O_RDONLY)
...
def multi_thread_safetensors_weights_iterator(
    ...
    def _load_file(st_file: str):
        ...
        # Evict page cache (Grace Blackwell unified memory)
        try:
            _fd = os.open(st_file, os.O_RDONLY)
            os.posix_fadvise(_fd, 0, 0, os.POSIX_FADV_DONTNEED)
            os.close(_fd)
