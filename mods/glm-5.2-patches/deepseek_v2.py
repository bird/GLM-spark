...
def _load_reap_mapping(model_path):
    """Load REAP pruning plan and build per-layer expert ID mapping."""
    import json
    import os
    plan_path = os.path.join(model_path, "reap-prune-plan-full.json")
    if not os.path.isfile(plan_path):
        return None
    with open(plan_path) as f:
        plan = json.load(f)
    mapping = {}
    for layer_id_str, layer_data in plan.items():
        kept = sorted(layer_data["kept"])
        layer_id = int(layer_id_str)
        mapping[layer_id] = {orig: new for new, orig in enumerate(kept)}
    return mapping


def _reap_remapped_weights(weights, reap_mapping):
    """Generator that remaps expert IDs in weight names for REAP-pruned models."""
    if reap_mapping is None:
        yield from weights
        return
    for name, weight in weights:
        if ".mlp.experts." in name:
            ...
            try:
                ...
                yield name, weight
                continue
            if layer_idx in reap_mapping and expert_idx in reap_mapping[layer_idx]:
                new_idx = reap_mapping[layer_idx][expert_idx]
                parts[parts.index("experts") + 1] = str(new_idx)
                name = ".".join(parts)
            else:
                continue
        elif ".mlp.gate." in name and (name.endswith(".weight") or name.endswith(".e_score_correction_bias")):
            parts = name.split(".")
            try:
                layer_idx = int(parts[parts.index("layers") + 1])
            except (ValueError, IndexError):
                yield name, weight
                continue
            if layer_idx in reap_mapping and weight.shape[0] > len(reap_mapping[layer_idx]):
                mapping = reap_mapping[layer_idx]
                kept_ids = sorted(mapping.keys())
                weight = weight[kept_ids]
        yield name, weight
...
def _try_load_fp8_indexer_wk(name, tensor, buf, params_dict, loaded_params):
    ...
    if fused_name not in params_dict:
        return True  # DSA disabled, skip indexer weight
...
class DeepseekV2MLAAttention(nn.Module):
    ...
    def __init__(
        ...
        self.is_v32 = hasattr(config, "index_topk") and os.environ.get("VLLM_DISABLE_DSA", "0") != "1" and os.environ.get("VLLM_DISABLE_DSA", "0") != "1" and os.environ.get("VLLM_DISABLE_DSA", "0") != "1"
...
class DeepseekV2Model(nn.Module):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        # REAP expert ID remapping
        import os as _os
        _model_path = getattr(self, "model_path", None) or _os.environ.get("VLLM_MODEL_PATH", "")
        _reap_mapping = None
        if _model_path and _os.path.isfile(_os.path.join(_model_path, "reap-prune-plan-full.json")):
            _reap_mapping = _load_reap_mapping(_model_path)
        if _reap_mapping is not None:
            weights = _reap_remapped_weights(weights, _reap_mapping)
